################################################################################
# CQRNDs2_3diffsize: Circular Quasi Rees neighbor design for block of three different
# sizes.
...
# Balanced Neighbor Designs through  CQRNDs2 Generated with R. 
...
# Quasi Rees neighbor design for block of three different
# sizes.
################################################################################
...
minimal CQRNDs2 for", "v=" ,object$R[1], ",","k1=",object$R[2],
...
###################################################################
# H is an output object from CQRNDs2_3diffsize
...
design<-function(H){
  ...
  setMethod("show", "CWBND_design", function(object) {
    ...
    cat(row, "\n")
    cat("Following is minimal CQRNDs2 for", "v=" ,object$R[1], "and","k=",object$R[2], "\n")
    row <- paste(rep("=", 51), collapse = "")
    cat(row, "\n")
...
###############################################################################
# Examples: Using CQRNDs2_3diffsize function to obtain the set(s) of shifts
# for construction of Circular Quasi Rees neighbor design for block  of 
...
# Example for case#1
(H<-CGN2_3diffsize(k=c(5,4,3),i=2,D=1))
(design(H))
